...
egospeed = xosc.AbsoluteSpeedAction(25,xosc.TransitionDynamics(xosc.DynamicsShapes.sinusoidal,xosc.DynamicsDimension.time,8))
...
trigger = xosc.EntityTrigger('mytesttrigger',0.2,xosc.ConditionEdge.none,trigcond,targetname)

event = xosc.Event('myfirstevent',xosc.Priority.overwrite)
event.add_trigger(trigger)

sin_time = xosc.TransitionDynamics(xosc.DynamicsShapes.linear,xosc.DynamicsDimension.time,5)
action = xosc.RelativeLaneChangeAction(-8,targetname,xosc.TransitionDynamics(xosc.DynamicsShapes.linear,xosc.DynamicsDimension.time,6),-10)
...
stoptrigger = xosc.EntityTrigger('stop_simulation',0.5,xosc.ConditionEdge.rising,xosc.OffroadCondition(0.5),targetname,triggeringpoint='stop')
