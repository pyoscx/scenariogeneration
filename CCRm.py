...
def CCRm(ego_speedvalue,offset):
    ...
    targetstart = pyoscx.TeleportAction(pyoscx.LanePosition(100,0,-1,1))
    ...
    ego_action = pyoscx.AbsoluteSpeedAction(ego_speedvalue/3.6,pyoscx.TransitionDynamics(pyoscx.DynamicsShapes.linear,pyoscx.DynamicsDimension.time,3))
    ...
    target_action = pyoscx.AbsoluteSpeedAction(20/3.6,pyoscx.TransitionDynamics(pyoscx.DynamicsShapes.linear,pyoscx.DynamicsDimension.time,3))
    ...
    ego_tar = pyoscx.Maneuver('target man')
    ego_tar.add_event(event_tar)
    ...
    sb = pyoscx.StoryBoard(init,pyoscx.ValueTrigger('stop_simulation',0,pyoscx.ConditionEdge.rising,pyoscx.SimulationTimeCondition(30,pyoscx.Rule.greaterThan),'stop'))
    sb.add_story(story)
...
if __name__ == '__main__':
    all_egospeeds = [x for x in range(30,80,5)]
    all_offsets = [-50, -25, 0, 25, 50]
    sce = CCRm(all_egospeeds[5],all_offsets[1])
