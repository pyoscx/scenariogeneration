...
class _OscEnum(VersionBase):
    ...
    """
    def __init__(self, classname, name, min_minor_version=0, max_minor_version=_MINOR_VERSION):
    ...
    def get_name(self):
        ...
        if self.min_minor_version > self.version_minor:
            ...
            raise OpenSCENARIOVersionError(self.classname + '.' + self.name + ' is not part of OpenSCENARIO V' + str(self.version_major) + '.' + str(self.version_minor) + ', was deprecated in V' + str(self.version_major) + '.' + str(self.max_minor_version))
        return self.name
...
class RelativeDistanceType():
    ...
    cartesianDistance = _OscEnum('RelativeDistanceType','cartesianDistance',max_minor_version=0)
    euclidianDistance = _OscEnum('RelativeDistanceType','euclidianDistance',min_minor_version=1)
...
class LateralDisplacement():
    ...
    leftToReferencedEntity = _OscEnum('LateralDisplacement','leftToReferencedEntity',min_minor_version=1)
    rightToReferencedEntity = _OscEnum('LateralDisplacement','rightToReferencedEntity',min_minor_version=1)

class LongitudinalDisplacement():
    any = _OscEnum('LongitudinalDisplacement','any',min_minor_version=1)
    trailingReferencedEntity = _OscEnum('LongitudinalDisplacement','trailingReferencedEntity',min_minor_version=1)
    leadingReferencedEntity = _OscEnum('LongitudinalDisplacement','leadingReferencedEntity',min_minor_version=1)
