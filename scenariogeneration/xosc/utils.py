...
class MonitorDeclaration(VersionBase):
    """MonitorDeclaration creates a MonitorDeclaration element of OpenSCENARIO
    (valid from OpenSCENARIO V1.3).
    ...
    def __init__(self, name: str, value: bool) -> None:
        """Initializes the MonitorDeclaration.
        ...
        if not isinstance(name, str) or not isinstance(
            ...
            raise TypeError(
                "name must be an string and value must be a bool/str/int"
            )
        self.name = str(name)
    ...
    @staticmethod
    def parse(element: ET.Element) -> "MonitorDeclaration":
        """Parses the XML element of MonitorDeclaration.

        Parameters
        ----------
        element : ET.Element
            A MonitorDeclaration element (same as generated by the class
            itself).
        Returns
        ----------
        MonitorDeclaration
            An object of type MonitorDeclaration
        """
        if "name" not in element.attrib:
            raise ValueError("MonitorDeclaration must have a name attribute")
        if "value" not in element.attrib:
            raise ValueError("MonitorDeclaration must have a value attribute")
        return MonitorDeclaration(
            element.attrib["name"], convert_bool(element.attrib["value"])
        )

    def get_attributes(self) -> dict[str, str]:
        """Returns the attributes of the MonitorDeclaration as a dictionary.

        Returns
        -------
        dict
            A dictionary of all attributes of the MonitorDeclaration.
        """
        retdict = {}
        retdict["name"] = self.name
