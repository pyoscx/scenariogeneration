...
class AbsoluteSpeedAction(_PrivateActionType):
    ...
    def parse(element):
        ...
        """
        speed_element = element.find('LongitudinalAction/SpeedAction/SpeedActionTarget/AbsoluteTargetSpeed')
...
class RelativeSpeedAction(_PrivateActionType):
    ...
    """
    def __init__(self,speed,entity,transition_dynamics,valuetype=SpeedTargetValueType.delta,continuous=True):
    ...
    def parse(element):
        ...
        """
        speed_element = element.find('LongitudinalAction/SpeedAction/SpeedActionTarget/RelativeTargetSpeed')
        td_element = element.find('LongitudinalAction/SpeedAction/SpeedActionDynamics')
        speed = speed_element.attrib['value']
        entity = speed_element.attrib['entityRef']
        continuous = bool(speed_element.attrib['continuous'])
        valuetype = getattr(SpeedTargetValueType,speed_element.attrib['speedTargetValueType'])
        transition_dynamics = TransitionDynamics.parse(td_element)
        return RelativeSpeedAction(speed,entity,transition_dynamics,valuetype,continuous)

    def get_attributes(self):
        """ returns the attributes of the RelativeSpeedAction as a dict

        """
        return {'entityRef':self.target,'value':str(self.speed),'speedTargetValueType':self.valuetype.get_name(),'continuous':convert_bool(self.continuous)}
...
class ControllerAction(_PrivateActionType):
    ...
    def __init__(self, assignControllerAction= None, overrideControllerValueAction=None, activateControllerAction=None):
        ...
        """
        if assignControllerAction is not None and not isinstance(assignControllerAction, AssignControllerAction):
            raise TypeError('assignControllerAction is not of type AssignControllerAction')
        if overrideControllerValueAction is not None and not isinstance(overrideControllerValueAction, OverrideControllerValueAction):
            raise TypeError('overrideControllerValueAction is not of type OverrideControllerValueAction')
        if activateControllerAction is not None and not isinstance(activateControllerAction, ActivateControllerAction):
            raise TypeError('activateControllerAction is not of type ActivateControllerAction')
    ...
    def __eq__(self, other):
        if isinstance(other,ControllerAction):
            if self.activateControllerAction == other.activateControllerAction and \
            self.overrideControllerValueAction == other.overrideControllerValueAction and \
            self.assignControllerAction == other.assignControllerAction:
                return True
    ...
    def __eq__(self,other) :
        if isinstance(other,OverrideControllerValueAction):
            ...
            self.parkingbrake_value == other.parkingbrake_value:
             ...
             if self.throttle_value == other.overrideControllerValueAction.throttle_value and \
            self.throttle_value == other.overrideControllerValueAction.throttle_value and \
            self.brake_active == other.overrideControllerValueAction.brake_active and \
            self.brake_value == other.overrideControllerValueAction.brake_value and \
            self.clutch_active == other.overrideControllerValueAction.clutch_active and \
            self.clutch_value == other.overrideControllerValueAction.clutch_value and \
            self.steeringwheel_active == other.overrideControllerValueAction.steeringwheel_active and \
            self.steeringwheel_value == other.overrideControllerValueAction.steeringwheel_value and \
            self.gear_active == other.overrideControllerValueAction.gear_active and \
            self.gear_value == other.overrideControllerValueAction.gear_value and \
            self.parkingbrake_active == other.overrideControllerValueAction.parkingbrake_active and \
            self.parkingbrake_value == other.overrideControllerValueAction.parkingbrake_value:
                return True
        return False
...
class TrafficSourceAction(_ActionType):
    ...
    def parse(element):
        ...
        return TrafficSourceAction(rate,radius,position,trafficdefinition,velocity,name)
