...
def osc_fixture():
    ...
    parameters.add_parameter(
        xosc.Parameter("param1", xosc.ParameterType.string, "hej")
    )
    parameters.add_parameter(
        xosc.Parameter("param2", xosc.ParameterType.integer, 1)
    )
    parameters.add_parameter(
        xosc.Parameter("param3", xosc.ParameterType.boolean, True)
    )
    ...
    veh = xosc.Vehicle(
        "mycar", xosc.VehicleCategory.car, bb, fa, ba, 150, 10, 10
    )
...
def test_catalog_reader_vehicle(tmpdir):
    ...
    orig.add_parameter(
        xosc.Parameter("asdf", xosc.ParameterType.string, "hej")
    )
    cf.add_to_catalog(orig)
    cf.dump()
    read = xosc.CatalogReader(
        xosc.CatalogReference("my_catalog", "car_white"), tmpdir
    )
...
def test_misc_object_reader(tmpdir):
    ...
    cf.create_catalog(
        tmpcatalog,
        "MiscObjectCatalog",
        "My first miscobject catalog",
        "Mandolin",
    )
    orig = xosc.MiscObject(
        "pole",
        50,
        xosc.MiscObjectCategory.pole,
        xosc.BoundingBox(1, 1, 1, 1, 1, 1),
    )
    cf.add_to_catalog(orig)
    cf.dump()
    read = xosc.CatalogReader(
        xosc.CatalogReference("my_catalog", "pole"), tmpdir
    )
    assert read == orig
...
def test_route_reader(tmpdir):
    ...
    cf.dump()
    read = xosc.CatalogReader(
        xosc.CatalogReference("my_catalog", "my_route"), tmpdir
    )
    assert read == orig
...
def test_environment_reader(tmpdir):
    ...
    cf.dump()
    read = xosc.CatalogReader(
        xosc.CatalogReference("my_catalog", "snow"), tmpdir
    )
    assert read == orig
